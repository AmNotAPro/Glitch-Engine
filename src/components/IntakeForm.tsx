...
const IntakeForm = ({ onComplete }: IntakeFormProps) => {
  ...
  if (!validateCurrentStep()) {
    console.log('I am here', validateCurrentStep()); 
    return;
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (selectedDate && selectedTime && jobResult) {
        ...
        console.log('Date time selecterd');
        const meetingData = {
          user_id: user.id,
  ...
  switch (currentStep) {
    ...
    case 1: // Page 2 - Role Details
      return (
        formData.startupJourney.trim() !== '' &&
        formData.roleType.trim() !== '' &&
        formData.keyResponsibilities.trim() !== '' &&
        formData.languagesRequired.length > 0 &&
        formData.skillsExperience.trim() !== ''
      );
    case 2: // Page 3 - Availability & Duration
      return (
    ...
    case 3: // Page 4 - Additional Info
     return (
       formData.workStyle.trim() !== '' &&
       formData.biggestChallenge.length > 0 &&
       meetingScheduled // <-- new boolean flag
  );
